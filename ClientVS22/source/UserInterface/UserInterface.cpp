...
static const char* GetLaunchErrorMessage()
{
	LANGID langId = PRIMARYLANGID(GetUserDefaultUILanguage());
	switch (langId)
	{
	case LANG_GERMAN:
		return "Das Spiel muss uber den Autopatcher gestartet werden.\nBei Problemen wenden Sie sich bitte an den Support: contact@metin2ignition.eu";
	case LANG_ROMANIAN:
		return "Jocul trebuie pornit cu ajutorul autopatcherului.\nIn caz de probleme va rog contactati supportul: contact@metin2ignition.eu";
	case LANG_TURKISH:
		return "Oyun otomatik guncelleme araci uzerinden baslatilmalidir.\nSorun yasarsaniz destek ile iletisime gecin: contact@metin2ignition.eu";
	case LANG_SPANISH:
		return "El juego debe iniciarse a traves del autopatcher.\nPara problemas, contacta con soporte: contact@metin2ignition.eu";
	case LANG_FRENCH:
		return "Le jeu doit etre lance via l'autopatcher.\nEn cas de probleme, veuillez contacter le support: contact@metin2ignition.eu";
	case LANG_ITALIAN:
		return "Il gioco deve essere avviato tramite l'autopatcher.\nPer problemi, contattare il supporto: contact@metin2ignition.eu";
	case LANG_POLISH:
		return "Gra musi byc uruchomiona przez autopatcher.\nW razie problemow skontaktuj sie z supportem: contact@metin2ignition.eu";
	case LANG_PORTUGUESE:
		return "O jogo deve ser iniciado pelo autopatcher.\nEm caso de problemas, contacte o suporte: contact@metin2ignition.eu";
	case LANG_DUTCH:
		return "Het spel moet worden gestart via de autopatcher.\nNeem bij problemen contact op met support: contact@metin2ignition.eu";
	case LANG_HUNGARIAN:
		return "A jatekot az autopatcheren keresztul kell elinditani.\nProblemak eseten lepjen kapcsolatba a supporttal: contact@metin2ignition.eu";
	case LANG_CZECH:
		return "Hru je nutne spustit prostrednictvim autopatcheru.\nV pripade problemu kontaktujte podporu: contact@metin2ignition.eu";
	case LANG_DANISH:
		return "Spillet skal startes via autopatcheren.\nVed problemer bedes du kontakte support: contact@metin2ignition.eu";
	case LANG_GREEK:
		return "To paichnidi prepei na ekkinithei mesa apo ton autopatcher.\nGia problimata epikoinoniste me to support: contact@metin2ignition.eu";
	case LANG_RUSSIAN:
		return "Igra dolzhna zapuskat'sya cherez avtopatcher.\nPri problemah svyazhites' so sluzhboj podderzhki: contact@metin2ignition.eu";
	default:
		return "The game must be started using the autopatcher.\nFor issues, please contact support: contact@metin2ignition.eu";
	}
}

static bool ValidateLaunchToken(LPSTR lpCmdLine)
{
	// Extract --launch-token <value> from command line
	const char* tokenArg = strstr(lpCmdLine, "--launch-token ");
	if (!tokenArg)
		return false;

	char token[64] = {};
	if (sscanf(tokenArg + strlen("--launch-token "), "%63s", token) != 1)
		return false;

	// Read launch.tok written by the patcher
	FILE* fp = fopen("launch.tok", "rt");
	if (!fp)
		return false;

	char fileToken[64] = {};
	long long fileTimestamp = 0;
	int read = fscanf(fp, "%63s\n%lld", fileToken, &fileTimestamp);
	fclose(fp);

	// Delete the file immediately (one-time use)
	DeleteFileA("launch.tok");
	...
	long long nowUnix = (long long)(ull.QuadPart / 10000000ULL) - 11644473600LL;

	if (nowUnix - fileTimestamp > 30LL)
		return false;
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (!ValidateLaunchToken(lpCmdLine))
	{
		MessageBoxA(NULL, GetLaunchErrorMessage(), "Metin2 Ignition", MB_OK | MB_ICONINFORMATION);
		return 0;
	}
